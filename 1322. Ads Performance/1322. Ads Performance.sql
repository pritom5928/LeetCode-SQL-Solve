...
SELECT 
	DISTINCT p.ad_id, 
	IF(s.ctr IS NULL , 0, s.ctr)  AS ctr
FROM ads p LEFT JOIN 
(	
	SELECT 
		a.totalid ,
		ROUND((b.ClickTotal / a.Total)*100,2) AS ctr   
	FROM cte2 a JOIN cte3 b ON a.TotalId = b.ClickId
	GROUP BY a.totalid
	ORDER BY ctr DESC, a.totalid ASC
) AS s 
